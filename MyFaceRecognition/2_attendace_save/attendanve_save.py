    ...
    for face_encoding in face_encodings:
        ...
        if distance < confidence_threshold:
            ...
            for face_location in face_locations:
                ...
                cv.putText(frame, 'Manas', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
            ...
            new_entry = pd.DataFrame({'Name': ['Manas'], 'Date': [date_str], 'Time': [time_str]})
            df = pd.concat([df, new_entry], ignore_index=True)

    if not recognized:
        cv.putText(frame, 'Not Manas', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                   (255, 0, 0), 2, cv.LINE_AA)
...
if not df.empty:
    df.to_excel('attendance_save.xlsx', index=False)
